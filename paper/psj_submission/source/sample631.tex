...
\begin{abstract}
Global lunar chemical maps are essential for understanding the origin and evolution of the Moon, its surface characteristics, and its potential for resource extraction. \revone{While lunar elemental abundances have been successfully derived using model-dependent X-ray spectroscopy, these methods often require concurrent high-quality solar monitoring and intensive computational modeling, which can limit spatial coverage.} Here we used X-ray fluorescence line intensity of O, Mg, Al, Si, Ca and Fe derived from five years of data from the Chandrayaan-2 Large Area Soft X-ray Spectrometer (CLASS) to generate global O/Si, Mg/Si, Al/Si, Mg/Al, Ca/Si and Fe/Si line intensity ratio maps at a resolution of 5.3~km/pixel. We have developed an independent data analysis methodology for CLASS, based on open source Python packages. \revone{Our analysis shows that Mg/Al and Al/Si line intensity ratios serve as effective empirical proxies for major geochemical terranes, showing good correlation with published elemental maps, returned sample data, and the Apollo 15 and 16 X-ray Fluorescence Spectrometer (XRS) maps.}
...
\revone{Typical light curves with the $N$-sigma, and standard $5$-sigma detection thresholds are shown in Figure~\ref{fig:lc_samples}. The $N$-sigma threshold defined by Equation~\ref{eq:detection-criteria1} identifies far more detections during periods of low solar activity compared to the $5$-sigma threshold.}
...
Figure \ref{fig:detection-algo} illustrates the steps described here.  There are 238,817 XRF line detections that pass our set of criteria (Equations~\ref{eq:detection-criteria2a}--\ref{eq:detection-criteria2c}) from November 2019 to May 2025. As solar activity increases, there are more detections, especially for elements with higher atomic numbers. After processing the data, we observed that the signal-to-noise ratio was low for some of the line ratios. Hence, calculations for Mg/Si, Fe$_L$/Si, O/Si, and Ca-K$\alpha$ lines were repeated using  296~s binning.
\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{find_flares_detection.jpeg}
    \caption{Light curves during a typical day during \revone{low solar activity} (\revone{top}) and \revone{high solar activity} (\revone{bottom}).}
...
We pass $std(\mathcal{L}^s_t)$ obtained from \revone{Equation}~\ref{eq:errors2} to the $\texttt{Spectrum1D}$ object, which then weighs each data point accordingly. These weights are used to decide the optimal fit and are propagated in the fitted parameters.
...
\begin{figure}
    ...
    \caption{Comparing XRF line intensity ratios \revone{(this work)} with elemental abundance data. Purple points denote inferred elemental weight ratios from \citet{NARENDRANATH2024115898}. The star symbols denote actual measurements from various missions. We see a positive linear correlation in the measurements. \revone{The dashed lines denote linear regression 1-D polynomial fits. The red dashed lines denote the fit to CLASS data, while the black dash-dotted line denotes the fit to elemental weight ratio data from various missions.}}
...
\revone{We hypothesize that the continuous multi-modal distribution observed in the data arises from underlying, independent populations corresponding to geographically diverse lunar terranes. By applying Gaussian Mixture Models (GMMs), we seek to capture and isolate these distinct geographic regions purely through their separation in compositional space.}

\revone{Mineralogically, these statistically defined populations are intended to correspond to the dominant lunar surface materials, primarily mapping the transition between basaltic mineral suites (which characterize the lunar maria and the South Pole-Aitken basin) and anorthositic mineral suites (which dominate the feldspathic highlands).}

\subsection{Clustering with Gaussian Mixture Models}
\revone{To quantify the clustering observed in the elemental line ratio data, we applied an $N$-component Gaussian Mixture Model to the two-dimensional line ratio distributions (e.g., $\mathcal{R}^\text{Mg}_\text{Si}$ versus $\mathcal{R}^\text{Al}_\text{Si}$). Mathematically, the GMM represents the overall probability density of an observed data point as a weighted linear combination of $N$ component multivariate Gaussian distributions \citep{Reynolds2009}. Based on the GMM model, every data point is assigned a posterior probability of belonging to each of the $N$ components, which is then used to assign the color mapping projected onto the lunar surface.}

\revone{While the Bayesian Information Criterion identifies $N=2$ as the optimal number of components for our data, this represents the primary binary dichotomy between the lunar highlands and mare. To better resolve the Moon's geochemical complexity, we also evaluate $N=3$ and $N=5$ configurations, which} \revtwo{can potentially indicate transitional mixing zones. $N=3$ is the highest component count for which the clusters correspond to distinct geological units, while $N=5$ is included to illustrate the continuity of the transitions between them.}


\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{Mg_Si_Al_Si_histograms.jpeg}
    \caption{\textit{Top panel:} A histogram of Al/Si line ratios \revone{(purple)} measured over the entire surface shows a clear bimodal distribution, corresponding to the Mare and highland terranes. \revone{The Gaussian fits to this distribution are shown by the dash-dot orange line.} \textit{Bottom panel:} A two-dimensional distribution of the Mg/Si and Al/Si line ratios also shows a multi-modal distribution. \revone{The colormap shows the density of data points in the 2D space, going fromm low (blue) to high (red) density.}}
    \label{fig:histograms}
\end{figure}

The most powerful combination of ratios, according to our analysis, is the 2D \revone{distribution} of $\mathcal{R}^\text{Mg}_\text{Si}$ and $\mathcal{R}^\text{Al}_\text{Si}$ \revone{(Figure~\ref{fig:histograms}, bottom panel)} which easily bifurcates into a region of high Al and low Al, as expected. 


The high Al/Si data points \revone{($\mathcal{R}^\text{Al}_\text{Si} \gtrsim 1.2$)} also show two distinct clusters \revone{(blue and green)}, which correspond to selenographically different regions (\revone{top panel, }Figure~\ref{fig:maps3}). Plotting on the lunar mercator projection with this same color scheme, we can see three regions separated out (1) The Basaltic Lunar Maria (2) South Pole Aitken (SPA) Basin (3) the Feldspathic Highlands terrane.  We apply a \revone{5}-component GMM which further distinguishes a low Mg/Si cluster in the north west highland Terranne and a low Mg/Si cluster in the north east \revone{(Figure~\ref{fig:maps3}, middle panel)}. Both models show the intermediate Mg/Si material that outlines the mare and the SPA. 
...
\begin{figure*}[htbp]
    ...
    \caption{\revone{Lunar geochemical classification using Gaussian Mixture Models (GMM) on CLASS XRF line intensity ratios. In all maps, black contours represent mare boundaries from LROC data. Left panels show the 2D density distributions and GMM cluster assignments; right panels show the corresponding spatial distribution on a global Mercator projection.
    \textit{Top row:} 3-component GMM applied to $\mathcal{R}^\text{Al}_\text{Si}$ vs. $\mathcal{R}^\text{Mg}_\text{Si}$ identifying the primary dichotomy between the Basaltic Maria (orange), and the Feldspathic Highlands (green), along with some transitional regions (blue) outlining the Maria and the South Pole Aitken basin.
    \textit{Middle row:} 5-component GMM of $\mathcal{R}^\text{Al}_\text{Si}$ vs. $\mathcal{R}^\text{Mg}_\text{Si}$, further resolving geochemical sub-units and mixing zones. 
    \textit{Bottom row:} 5-component GMM applied to the distribution of the Mg\# proxy derived from $(\mathcal{R}^\text{Mg}_\text{Si} / [\mathcal{R}^\text{Mg}_\text{Si} + \mathcal{R}^\text{Fe}_\text{Si}])$,} \revtwo{grouping Mare Moscoviense, Australe, Marginis, and Smythii separately from the larger nearside basins.}}
    \label{fig:maps3}
\end{figure*}

We define an analogous quantity to the Magnesium Number (Mg\#) as $\mathcal R^\text{Mg}_\text{Si}/\left({R^\text{Mg}_\text{Si}+R^\text{Fe}_\text{Si}}\right)$  and apply a \revone{5}-component GMM \revone{(Figure~\ref{fig:maps3}, bottom panel)}. Lower Mg\# ratios appear in the mare and higher in highland terrane with a transition \revone{in} between, as observed in previously published Mg\# maps \citep{WANG2021105360,ZHANG2023115505}. The large basins on the near side belong to the lowest Mg\# cluster suggesting younger lava emplacements. In particular, isolated mare, \revtwo{(Moscoviense, Australe, Marginis, and Smythii) are grouped separately from the larger nearside basins. This grouping follows from their similar Al/Si values and does not imply that they are geochemically similar (Figure~\ref{fig:maps1}).}

\subsection{Global maps}

We have generated global high-resolution maps of major elements with XRF line intensity ratios from CLASS on Chandrayaan-2. The Mg/Al and Al/Si maps are particularly useful to identify geochemically distinct regions that can be used for further detailed studies. The Mg/Al line intensity map is plotted with the contours of the mare basalts\revone{, along with notable geography commonly referenced} in Figure~\ref{fig:Mg_GIS}.

The highest Mg/Al regions are associated with the near-side mare basins. Notable regions are the \revone{Aristarchus} plateau and a region close to the boundary of \revone{Oceanus Procellarum} towards the north east. Although the anomalously high Mg/Al ratio ($\approx$ 2 compared to the global average of 0.75) north west of the Aristarchus crater coincides with high olivine abundances on the map derived from the Kaguya multiband imager \citep{2019P&SS..165..230L}, other similar regions do not. These regions are likely to be distinct in mafic content and composition, which requires detailed studies.
...
\begin{table}[!t]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        Region & Al/Si& Mg/Si& Mg/Al\\ \hline
        North Polar   & $1.48 \pm 0.31$ & $0.99 \pm 0.11$   & $0.64 \pm 0.09$ \\ \hline
        South Polar   & $1.36 \pm 0.36$   & $1.05 \pm 0.12$   & $0.73 \pm 0.12$ \\ \hline
        Highlands & $1.59 \pm 0.20$ & $1.14 \pm 0.14$ & $0.69 \pm 0.11$ \\ \hline
        Mare & $1.18 \pm 0.17$ & $1.12 \pm 0.14$ & $0.96 \pm 0.22$ \\ \hline
    \end{tabular}
    \caption{Average values of elemental line ratios in the lunar polar regions ($\pm70^\circ\text{ to }\pm90^\circ$), Highlands and Mare.}
    \label{tab:average}
\end{table}
...
\begin{table}[!t]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        Site & Ratio & Apollo & This Work & \revtwo{This/Apollo} \\
        \hline
        \multirow{3}{1.6cm}{Mare Tranquil\-litatis}
            & Al/Si & $0.52 \pm 0.10$ & $1.00 \pm 0.03$ & \revtwo{$1.92 \pm 0.37$} \\
            & Mg/Si & $0.62 \pm 0.15$ & $1.11 \pm 0.05$ & \revtwo{$1.79 \pm 0.44$} \\
            & Mg/Al & $1.15 \pm 0.24$ & $1.10 \pm 0.03$ & \revtwo{$0.96 \pm 0.20$} \\
        \hline
        \multirow{3}{1.6cm}{Mare Fecunditatis}
            & Al/Si & $0.58 \pm 0.13$ & $1.10 \pm 0.04$ & \revtwo{$1.90 \pm 0.43$} \\
            & Mg/Si & $0.51 \pm 0.14$ & $1.20 \pm 0.02$ & \revtwo{$2.35 \pm 0.65$} \\
            & Mg/Al & $0.89 \pm 0.29$ & $1.12 \pm 0.03$ & \revtwo{$1.26 \pm 0.41$} \\
        \hline
        \multirow{3}{1.6cm}{Hadley-Apennines}
            & Al/Si & $0.53 \pm 0.19$ & $1.09 \pm 0.03$ & \revtwo{$2.06 \pm 0.74$} \\
            & Mg/Si & $0.64 \pm 0.20$ & $1.31 \pm 0.05$ & \revtwo{$2.05 \pm 0.64$} \\
            & Mg/Al & $1.26 \pm 0.30$ & $1.18 \pm 0.01$ & \revtwo{$0.94 \pm 0.22$} \\
        \hline
        \multirow{3}{1.6cm}{Descartes Highlands}
            & Al/Si & $0.81 \pm 0.11$ & $1.57 \pm 0.09$ & \revtwo{$1.94 \pm 0.29$} \\
            & Mg/Si & $0.40 \pm 0.14$ & $1.32 \pm 0.03$ & \revtwo{$3.30 \pm 1.16$} \\
            & Mg/Al & $0.49 \pm 0.20$ & $0.79 \pm 0.01$ & \revtwo{$1.61 \pm 0.66$} \\
        \hline
        \multirow{3}{1.6cm}{Taurus-Littrow}
            & Al/Si & $0.55 \pm 0.09$ & $1.12 \pm 0.03$ & \revtwo{$2.04 \pm 0.34$} \\
            & Mg/Si & $0.55 \pm 0.12$ & $1.15 \pm 0.07$ & \revtwo{$2.09 \pm 0.47$} \\
            & Mg/Al & $1.01 \pm 0.26$ & $1.06 \pm 0.04$ & \revtwo{$1.05 \pm 0.27$} \\
        \hline
    \end{tabular}
    \caption{Comparison of Al/Si, Mg/Si, and Mg/Al ratios between Apollo XRFS measurements and this work at the five sites mentioned in Table 3 of \citet{Gloudermans2021}. \revtwo{The last column gives the ratio of our values to the Apollo values, with propagated uncertainties. The weighted means across the five sites are $1.96 \pm 0.17$ for Al/Si, $2.08 \pm 0.26$ for Mg/Si and $1.02 \pm 0.12$ for Mg/Al.}}
...
Table~\ref{tab:average} gives the average values of Mg/Si, Al/Si and Mg/Al ratios for the polar regions, highlands and mare. \revone{As seen from Table~\ref{tab:apollo_comparison}, our values for the Al/Si and Mg/Si ratios are consistently higher than those derived from Apollo XRFS \citep{Gloudermans2021}.} \revtwo{The offset is a constant factor across the five sites, with weighted means of $1.96 \pm 0.17$ for Al/Si and $2.08 \pm 0.26$ for Mg/Si, rather than a terrane dependent difference. It likely arises because the CLASS quantum efficiency is reduced at the Si-K$\alpha$ line at $1.74$~keV, which falls between the Al K-edge of the optical blocking filter at $1.56$~keV and the Si K-edge of the detector at $1.84$~keV \citep[Figure~8]{PILLAI2021114436}, underestimating the Si line flux.} \revtwo{The gas proportional counters used in the Apollo missions are free of this effect.} \revtwo{Mg/Al, for which both line energies lie below the Al K-edge, is $1.02 \pm 0.12$ and shows no such offset.}
...
\section{Summary}
We have developed a processing methodology to detect XRF signals from the CLASS calibrated spectral files and generate high-resolution maps of line intensity ratios \revtwo{from five years of data}. \revone{Unlike conventional analyses that rely on forward modeling of incident solar spectra to derive absolute abundances, our approach utilizes raw XRF line intensity ratios as a proxy for surface composition.} \revtwo{This provides a useful first order picture of the global distribution of lunar elements. Since the method requires neither concurrent solar measurements nor spectra with sufficient statistics for a full spectral fit, a much larger fraction of the observations becomes usable, allowing these maps to be generated at a finer spatial scale.} The Mg/Al and Al/Si line intensity ratio \revone{maps show positive correlations} with abundance ratios in wt\% from return samples, as well as published CLASS elemental maps. The Mg/Al maps best represent the geochemical diversity that shows distinct regions that would be of interest for further, detailed studies.
