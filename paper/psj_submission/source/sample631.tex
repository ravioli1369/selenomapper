...
\definecolor{trackchange}{rgb}{0.75,0,0}
\definecolor{explain}{rgb}{0,0,0.7}
\submitjournal{PSJ}
...
\revone{While the Bayesian Information Criterion identifies $N=2$ as the optimal number of components for our data, this represents the primary binary dichotomy between the lunar highlands and mare. To better resolve the Moon's geochemical complexity, we also evaluate $N=3$ and $N=5$ configurations, which identify transitional mixing zones and secondary lithologies that provide critical insights into the localized evolution of the lunar crust.}
...
\begin{figure*}[htbp]
    ...
    \textit{Bottom row:} 5-component GMM applied to the distribution of the Mg\# proxy derived from $(\mathcal{R}^\text{Mg}_\text{Si} / [\mathcal{R}^\text{Mg}_\text{Si} + \mathcal{R}^\text{Fe}_\text{Si}])$, showing previously unseen separation of Mare Moscoviense, Australe, Marginis, and Smythii.}}
    \label{fig:maps3}
\end{figure*}

We define an analogous quantity to the Magnesium Number (Mg\#) as $\mathcal R^\text{Mg}_\text{Si}/\left({R^\text{Mg}_\text{Si}+R^\text{Fe}_\text{Si}}\right)$  and apply a \revone{5}-component GMM \revone{(Figure~\ref{fig:maps3}, bottom panel)}. Lower Mg\# ratios appear in the mare and higher in highland terrane with a transition \revone{in} between, as observed in previously published Mg\# maps \citep{WANG2021105360,ZHANG2023115505}. The large basins on the near side belong to the lowest Mg\# cluster suggesting younger lava emplacements. In particular, isolated mare, \revone{(Moscoviense, Australe, Marginis, and Smythii) now appear as distinct clusters.}
...
Table~\ref{tab:average} gives the average values of Mg/Si, Al/Si and Mg/Al ratios for the polar regions, highlands and mare. \revone{As seen from Table~\ref{tab:apollo_comparison}, our values for the Al/Si and Mg/Si ratios are consistently higher ($\sim 2\times$) than those derived from Apollo XRFS \citep{Gloudermans2021}. This is likely because of the systematic underestimation of Si line flux in the Si-based CLASS detectors due to reduced quantum efficiency at the Si K-edge \citep{PILLAI2021114436}. Since the gas proportional counters used in the Apollo missions are free of this effect, our Si-normalized ratios are consistently shifted toward higher absolute values compared to the Apollo baseline.}
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|c|}
        \hline
        Site & Ratio & Apollo XRFS & This Work \\
        \hline
        \multirow{3}{*}{Mare Tranquillitatis}
            & Al/Si & $0.52 \pm 0.10$ & $1.00 \pm 0.03$ \\
            & Mg/Si & $0.62 \pm 0.15$ & $1.11 \pm 0.05$ \\
            & Mg/Al & $1.15 \pm 0.24$ & $1.10 \pm 0.03$ \\
        \hline
        \multirow{3}{*}{Mare Fecunditatis}
            & Al/Si & $0.58 \pm 0.13$ & $1.10 \pm 0.04$ \\
            & Mg/Si & $0.51 \pm 0.14$ & $1.20 \pm 0.02$ \\
            & Mg/Al & $0.89 \pm 0.29$ & $1.12 \pm 0.03$ \\
        \hline
        \multirow{3}{*}{Hadley-Apennines}
            & Al/Si & $0.53 \pm 0.19$ & $1.09 \pm 0.03$ \\
            & Mg/Si & $0.64 \pm 0.20$ & $1.31 \pm 0.05$ \\
            & Mg/Al & $1.26 \pm 0.30$ & $1.18 \pm 0.01$ \\
        \hline
        \multirow{3}{*}{Descartes Highlands}
            & Al/Si & $0.81 \pm 0.11$ & $1.57 \pm 0.09$ \\
            & Mg/Si & $0.40 \pm 0.14$ & $1.32 \pm 0.03$ \\
            & Mg/Al & $0.49 \pm 0.20$ & $0.79 \pm 0.01$ \\
        \hline
        \multirow{3}{*}{Taurus-Littrow}
            & Al/Si & $0.55 \pm 0.09$ & $1.12 \pm 0.03$ \\
            & Mg/Si & $0.55 \pm 0.12$ & $1.15 \pm 0.07$ \\
            & Mg/Al & $1.01 \pm 0.26$ & $1.06 \pm 0.04$ \\
        \hline
    \end{tabular}
    \caption{Comparison of Al/Si, Mg/Si, and Mg/Al ratios between Apollo XRFS measurements and this work at the five sites mentioned in Table 3 of \citet{Gloudermans2021}.}
    \label{tab:apollo_comparison}
\end{table}


\section{Summary}
We have developed a processing methodology to detect XRF signals from the CLASS calibrated spectral files and generate high-resolution maps of line intensity ratios. \revone{Unlike conventional analyses that rely on forward modeling of incident solar spectra to derive absolute abundances, our approach utilizes raw XRF line intensity ratios as a first-order proxy for surface composition. We show that even a simplified, model-free analysis of XRF line ratios can yield meaningful insights into the global distribution of lunar elements.} Using five years of CLASS data, global line intensity ratio maps of major elements are generated. The Mg/Al and Al/Si line intensity ratio \revone{maps show positive correlations} with abundance ratios in wt\% from return samples, as well as published CLASS elemental maps. The Mg/Al maps best represent the geochemical diversity that shows distinct regions that would be of interest for further, detailed studies.
