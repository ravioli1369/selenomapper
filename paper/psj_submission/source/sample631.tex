...
\textbf{Typical light curves with the $N$-sigma, and standard $5$-sigma detection thresholds are shown in Figure~\ref{fig:lc_samples}. The $N$-sigma threshold defined by Equation~\ref{eq:detection-criteria1} identifies far more detections during periods of low solar activity compared to the $5$-sigma threshold.}
...
\begin{figure}[ht]
    ...
    \caption{Light curves during a typical day during \textbf{low solar activity} (\textbf{top}) and \textbf{high solar activity} (\textbf{bottom}).}
...
We pass $std(\mathcal{L}^s_t)$ obtained from \textbf{Equation}~\ref{eq:errors2} to the $\texttt{Spectrum1D}$ object, which then weighs each data point accordingly. These weights are used to decide the optimal fit and are propagated in the fitted parameters.
...
\textbf{Despite these systematic offsets, the relative trends remain robust enough to differentiate major geochemical units. Mg/Al line ratio values $\mathcal{R}^\text{Mg}_\text{Al} \lesssim 0.8$ generally indicate highland regions, while for Al/Si, non-mare regions typically show $\mathcal{R}^\text{Al}_\text{Si} \gtrsim 1.25$.}
...
\textbf{Mineralogically, these statistically defined populations are intended to correspond to the dominant lunar surface materials, primarily mapping the transition between basaltic mineral suites (which characterize the lunar maria and the South Pole-Aitken basin) and anorthositic mineral suites (which dominate the feldspathic highlands).}

\subsection{Clustering with Gaussian Mixture Models}
\textbf{To quantify the clustering observed in the elemental line ratio data, we applied an $N$-component Gaussian Mixture Model to the two-dimensional line ratio distributions (e.g., $\mathcal{R}^\text{Mg}_\text{Si}$ versus $\mathcal{R}^\text{Al}_\text{Si}$). Mathematically, the GMM represents the overall probability density of an observed data point as a weighted linear combination of $N$ component multivariate Gaussian distributions \citep{Reynolds2009}. Based on the GMM model, every data point is assigned a posterior probability of belonging to each of the $N$ components, which is then used to assign the color mapping projected onto the lunar surface.}

\textbf{While the Bayesian Information Criterion identifies $N=2$ as the optimal number of components for our data, this represents the primary binary dichotomy between the lunar highlands and mare. To better resolve the Moon's geochemical complexity, we also evaluate $N=3$ and $N=5$ configurations, which identify transitional mixing zones and secondary lithologies that provide critical insights into the localized evolution of the lunar crust.}


\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{Mg_Si_Al_Si_histograms.jpeg}
    \caption{\textit{Top panel:} A histogram of Al/Si line ratios \textbf{(purple)} measured over the entire surface shows a clear bimodal distribution, corresponding to the Mare and highland terranes. \textbf{The Gaussian fits to this distribution are shown by the dash-dot orange line.} \textit{Bottom panel:} A two-dimensional distribution of the Mg/Si and Al/Si line ratios also shows a multi-modal distribution. \textbf{The colormap shows the density of data points in the 2D space, going fromm low (blue) to high (red) density.}}
    \label{fig:histograms}
\end{figure}

The most powerful combination of ratios, according to our analysis, is the 2D \textbf{distribution} of $\mathcal{R}^\text{Mg}_\text{Si}$ and $\mathcal{R}^\text{Al}_\text{Si}$ \textbf{(Figure~\ref{fig:histograms}, bottom panel)} which easily bifurcates into a region of high Al and low Al, as expected. 


The high Al/Si data points \textbf{($\mathcal{R}^\text{Al}_\text{Si} \gtrsim 1.2$)} also show two distinct clusters \textbf{(blue and green)}, which correspond to selenographically different regions (\textbf{top panel, }Figure~\ref{fig:maps3}). Plotting on the lunar mercator projection with this same color scheme, we can see three regions separated out (1) The Basaltic Lunar Maria (2) South Pole Aitken (SPA) Basin (3) the Feldspathic Highlands terrane.  We apply a \textbf{5}-component GMM which further distinguishes a low Mg/Si cluster in the north west highland Terranne and a low Mg/Si cluster in the north east \textbf{(Figure~\ref{fig:maps3}, middle panel)}. Both models show the intermediate Mg/Si material that outlines the mare and the SPA. 
...
We define an analogous quantity to the Magnesium Number (Mg\#) as $\mathcal R^\text{Mg}_\text{Si}/\left({R^\text{Mg}_\text{Si}+R^\text{Fe}_\text{Si}}\right)$  and apply a \textbf{5}-component GMM \textbf{(Figure~\ref{fig:maps3}, bottom panel)}. Lower Mg\# ratios appear in the mare and higher in highland terrane with a transition \textbf{in} between, as observed in previously published Mg\# maps \citep{WANG2021105360,ZHANG2023115505}. The large basins on the near side belong to the lowest Mg\# cluster suggesting younger lava emplacements. In particular, isolated mare, \textbf{(Moscoviense, Australe, Marginis, and Smythii) now appear as distinct clusters.}

\subsection{Global maps}

We have generated global high-resolution maps of major elements with XRF line intensity ratios from CLASS on Chandrayaan-2. The Mg/Al and Al/Si maps are particularly useful to identify geochemically distinct regions that can be used for further detailed studies. The Mg/Al line intensity map is plotted with the contours of the mare basalts\textbf{, along with notable geography commonly referenced} in Figure~\ref{fig:Mg_GIS}.

The highest Mg/Al regions are associated with the near-side mare basins. Notable regions are the \textbf{Aristarchus} plateau and a region close to the boundary of \textbf{Oceanus Procellarum} towards the north east. Although the anomalously high Mg/Al ratio ($\approx$ 2 compared to the global average of 0.75) north west of the Aristarchus crater coincides with high olivine abundances on the map derived from the Kaguya multiband imager \citep{2019P&SS..165..230L}, other similar regions do not. These regions are likely to be distinct in mafic content and composition, which requires detailed studies.
...
Table~\ref{tab:average} gives the average values of Mg/Si, Al/Si and Mg/Al ratios for the polar regions, highlands and mare. \textbf{As seen from Table~\ref{tab:apollo_comparison}, our values for the Al/Si and Mg/Si ratios are consistently higher ($\sim 2\times$) than those derived from Apollo XRFS \citep{Gloudermans2021}. This is likely because of the systematic underestimation of Si line flux in the Si-based CLASS detectors due to reduced quantum efficiency at the Si K-edge \citep{PILLAI2021114436}. Since the gas proportional counters used in the Apollo missions are free of this effect, our Si-normalized ratios are consistently shifted toward higher absolute values compared to the Apollo baseline.}
